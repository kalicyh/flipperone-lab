...
scripts/docker-lab.sh          Docker build/run helper
...
```bash
./scripts/docker-lab.sh build-dev
./scripts/docker-lab.sh run-dev
```
...
The top-left version badge shows the `flipperone-testing` and `flipperone-linux-build-scripts` commit SHAs baked into the image.
...
```bash
./scripts/docker-lab.sh build-rootfs
./scripts/docker-lab.sh run-rootfs
```
...
Audio playback is also handled by the browser-side lab runtime. The upstream fake UI normally tries to play audio inside Linux with SoX and ALSA, but containers may not expose `/dev/snd` by default. The lab runtime intercepts sound playback calls and plays files from:
...
```bash
./scripts/docker-lab.sh stop
```

The Docker helper uses `linux/arm64` by default. Override it with `DOCKER_PLATFORM` if needed.
